/*
   ...
   org.xapagy.xapi.MacroCreateLink
...
public class KludgeCreateLink {
    ...
    enum LinkParseState {
        WAIT_FOR_TYPE, WAIT_FOR_FROM_BRACKET, WAIT_FOR_FROM_TYPE,
        WAIT_FOR_ARROW, WAIT_FOR_TO_BRACKET, WAIT_FOR_TO_TYPE, WAIT_FOR_END;
    };
    ...
    public static class CreateLinkSpec {
        ...
        public String toString() {
            return "CreateLinkSpec [linkName=" + linkName + ", vmfFrom="
                    + vmfFrom + ", vmfTo=" + vmfTo + ", strenght=" + strength
                    + "]";
        }
     ...
     * LinkAPI
     * 
     * @param cls - specifies the way to create a link
     * @param agent
     */
    public static void kludgeCreateLink(CreateLinkSpec cls, Agent agent) {
        ViMatch vim = new ViMatch(agent);
        List<VerbInstance> allFocusVis =
                agent.getFocus().getViListAllEnergies();
        List<VerbInstance> select = vim.select(allFocusVis, cls.vmfFrom);
        if (select.isEmpty()) {
            throw new Error("Could not find VI specified by: "
                    + cls.vmfFrom);
        }
        ...
        agent.getLinks().changeLinkByName(cls.linkName, from, to,
                cls.strength, "KludgeCreateLink");
    }
    ...
    public static CreateLinkSpec parseCreateLink(String text) {
        ...
        while (st.hasMoreElements()) {
            ...
            case WAIT_FOR_FROM_BRACKET:
                ...
                throw new Error("WAIT_FOR_FROM_BRACKET: Was waiting for { got"
                        + next);
            ...
            case WAIT_FOR_ARROW:
                ...
                throw new Error("WAIT_FOR_ARROW: Was expecting ==>, but got"
                        + next);
            case WAIT_FOR_TO_BRACKET:
                ...
                throw new Error("WAIT_FOR_TO_BRACKET: Was waiting for { got"
                        + next);
            ...
            case WAIT_FOR_END:
                ...
                try {
                    ...
                    throw new Error(
                            "WAIT_FOR_END: This was supposed to be a number"
                                    + next);
                }
                // throw new
