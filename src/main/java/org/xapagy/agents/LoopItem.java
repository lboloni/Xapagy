...
public class LoopItem implements XapagyComponent, Serializable {
	...
	public enum LoopItemType {
		EXTERNAL, INTERNAL, READING, FORCED
	};
	 ...
	 * @return
	 */
	public static LoopItem createInternal(Agent agent, double time, Choice choice, double willingness) {
	 ...
	 * @return
	 */
	public static LoopItem createForced(Agent agent, double time, VerbInstance forcedVi, double forcedTimeAfter) {
		LoopItem retval = new LoopItem(agent);
		retval.state = LoopItemState.NOT_EXECUTED;
		retval.type = LoopItemType.FORCED;
		retval.executionTime = time;
	 ...
	 * @return
	 */
	public static LoopItem createReading(Agent agent, String text, File file, int fileLineNo) {
		LoopItem retval = new LoopItem(agent);
		retval.state = LoopItemState.NOT_EXECUTED;
		retval.type = LoopItemType.READING;
		retval.text = text;
	 ...
	 * @return
	 */
	public static LoopItem createScheduled(Agent agent, String text, double time) {
		LoopItem retval = new LoopItem(agent);
		retval.state = LoopItemState.NOT_EXECUTED;
	...
	private void executeExternalOrReading() {
		...
		for (XapiReference statement : atomicStatements) {
			...
			if (statement.getType().equals(XapiReferenceType.WAIT)) {
				Execute.executeDiffusionActivities(agent, ((XrefWait) statement).getTimeWait());
				continue;
			}
