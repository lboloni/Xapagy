...
public class Agent implements Serializable {
    ...
    /**
     * An auxiliary class containing debugging information - should not affect the behavior of a functioning Xapagy agent
     */
    private AgentDebugInfo debugInfo = null;
    /**
     * An auxiliary class containing labeling information ($Define's and namespaces)
     */
    ...
    public Agent(String name) {
        ...
        //
        parameters.addParam("A_DEBUG", "G_GENERAL",
                "N_RECORD_CHOICES_INTO_LOOPITEM", false,
        ...
        firstScene.getConcepts()
                .addFullEnergy(getConceptDB().getConcept(Hardwired.C_SCENE));
     ...
     * @return
     */
    public VerbInstance createVerbInstance(ViType instanceType,
            VerbOverlay verbs) {
        String id = identifierGenerator.getVerbInstanceIdentifier();
        VerbInstance verbInstance =
                new VerbInstance(instanceType, id, verbs, this);
    ...
    public void notifyObservers(DebugEvent event) {
        ...
        for (IAgentObserver observer : observers.values()) {
            try {
                ...
                TextUi.println(
                        "Exception in observer, logged but proceed:" + e);
                e.printStackTrace();
            }
        }
