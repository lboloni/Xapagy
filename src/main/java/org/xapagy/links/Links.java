...
public class Links implements Serializable {
     ...
     * 
     */
    public void createRegularLinkType(String linkTypeName,
        ...
        p.addParam("A_SHM", "G_LINK_STRUCTURE", "N_WEIGHT-" + linkTypeName,
                defaultWeight, "Default weight of link type " + linkTypeName);
    }
     ...
     * 
     * @param linkTypeName
     *            the name of the "forward" newly created link name. Link names cannot be
     *            overwritten so an error is thrown in the name exists
     * @param defaultWeight
     *            the default weight "forward" link
     * @param reverseLinkTypeName
     *            the name of the "reverse" newly created link name. Link names cannot be
     *            overwritten so an error is thrown in the name exists
     * @param reverseWeight
     *            the default weight "reverse" link
     * 
     */
    public void createDirectionPairedLinkTypes(String linkTypeName,
            double defaultWeight, String reverseLinkTypeName, double reverseWeight, String description) {
        if (linkValues.keySet().contains(linkTypeName)) {
        ...
        p.addParam("A_SHM", "G_LINK_STRUCTURE", "N_WEIGHT-" + linkTypeName,
                defaultWeight, "Default weight of link type " + linkTypeName);
        p.addParam("A_SHM", "G_LINK_STRUCTURE", "N_WEIGHT-" + reverseLinkTypeName,
                reverseWeight, "Default weight of link type " + reverseLinkTypeName);
    }
     ...
     */
    void changeLink(String linkName, VerbInstance vi1, VerbInstance vi2,
            double delta) {
        ...
        if (!linkValues.isDirectionPaired()) {
            ...
            double existingValue =
                    linkValues.getLink(vi1, vi2) - irel.getLink(vi1, vi2);
            double newValue = existingValue + delta;
            if (newValue >= 0) {
                linkValues.putLink(vi1, vi2, newValue);
     ...
     * @param viBegin
     */
    public void removeElaborationBegin(VerbInstance fvi, VerbInstance viBegin,
            String source) {
        ViSet begins = getLinksByLinkName(fvi, Hardwired.LINK_ELABORATION_BEGIN);
        double value = begins.value(viBegin);
        changeLinkByName(Hardwired.LINK_ELABORATION_BEGIN, fvi, viBegin, -value,
                source + "+removeElaborationBegin");
    }
     ...
     * @return
     */
    public double getLinkValueByLinkName(VerbInstance from, VerbInstance to,
            String linkName) {
     ...
     * @param viBegin
     */
    public void removeElaborationBody(VerbInstance fvi, VerbInstance viBody,
            String source) {
        ViSet begins = getLinksByLinkName(fvi, Hardwired.LINK_ELABORATION_BODY);
        double value = begins.value(viBody);
        changeLinkByName(Hardwired.LINK_ELABORATION_BODY, fvi, viBody, -value, source
                + "+removeElaborationBody");
    }
     ...
     * @param delta
     */
    public void changeLinkByName(String linkName, VerbInstance viFrom,
            VerbInstance viTo, double delta, String source) {
        LinkQuantum lq =
                LinkQuantum
                        .createQuantum(linkName, viFrom, viTo, delta, source);
    ...
    private void applyQuantum(LinkQuantum lq) {
        ...
        if (recordLinkQuantum) {
            double val =
                    getLinkValueByLinkName(lq.getFrom(), lq.getTo(),
                            lq.getLinkName());
            lq.setValueBefore(val);
        }
        changeLink(lq.getLinkName(), lq.getFrom(), lq.getTo(),
                lq.getDelta());
        if (recordLinkQuantum) {
            double val =
                    getLinkValueByLinkName(lq.getFrom(), lq.getTo(),
     ...
     * @param viTo
     */
    public void copyAllLinks(VerbInstance viFrom, VerbInstance viTo,
            String source) {
        for (String linkName : getLinkTypeNames()) {
            ...
            for (SimpleEntry<VerbInstance, Double> entry : viset.getList()) {
                ...
                changeLinkByName(linkName, viTo, other, entry.getValue(),
                        source + "+copyAllLinks");
                ...
                if (linkName.equals(Hardwired.LINK_COINCIDENCE)) {
                    ...
                    changeLinkByName(linkName, other, viTo, entry.getValue(),
                            source + "+copyAllLinks(coincidence)");
                }
            }
        }
    }
