...
public class DaShmLinkStructure extends AbstractDaFocusIterator {
    ...
    /**
     * Applies the DA to every verb instance in the focus
     * 
     */
    @Override
    protected void applyFocusVi(VerbInstance fvi, double timeSlice) {
        ...
        //
        strength =
                scene.getSceneParameters().getLinkStructureWeight()
                        .get(Hardwired.LINK_SUCCESSOR);
        eqslink =
                applyForLinkType(fvi, timeSlice, Hardwired.LINK_SUCCESSOR, strength,
        ...
        strength =
                scene.getSceneParameters().getLinkStructureWeight()
                        .get(Hardwired.LINK_PREDECESSOR);
        eqslink =
                applyForLinkType(fvi, timeSlice, Hardwired.LINK_PREDECESSOR,
                        strength, EnergyColors.SHV_TEMPORAL_ORDER);
        eqs.addAll(eqslink);
        //
        // Coincidence links
        //
        strength =
                scene.getSceneParameters().getLinkStructureWeight()
                        .get(Hardwired.LINK_COINCIDENCE);
        eqslink =
                applyForLinkType(fvi, timeSlice, Hardwired.LINK_COINCIDENCE,
                        strength, EnergyColors.SHV_TEMPORAL_ORDER);
        eqs.addAll(eqslink);
     ...
     * @param linkType
     * @param strength
     *            - scaling parameter, based on
     * @param ec
     *            - the shadow energy which is added
     */
    protected List<EnergyQuantum<VerbInstance>> applyForLinkType(
            VerbInstance fvi, double timeSlice, String linkType,
            double strength, String ec) {
        List<EnergyQuantum<VerbInstance>> retval = new ArrayList<>();
        Links la = agent.getLinks();
        List<SimpleEntry<VerbInstance, Double>> fviLinks =
                la.getLinksByLinkName(fvi, linkType).getList();
        List<VerbInstance> shadows =
                sf.getMembers(fvi, EnergyColors.SHV_GENERIC);
        for (VerbInstance svi : shadows) {
            //double fvisviSalience =
            //        sf.getSalience(fvi, svi, EnergyColor.SHV_GENERIC);
            List<SimpleEntry<VerbInstance, Double>> sviLinks =
                    la.getLinksByLinkName(svi, linkType).getList();
            for (SimpleEntry<VerbInstance, Double> focusEntry : fviLinks) {
                for (SimpleEntry<VerbInstance, Double> shadowEntry : sviLinks) {
                    double linkStrength =
                            Math.min(focusEntry.getValue(),
                                    shadowEntry.getValue());
                    ...
                    EnergyQuantum<VerbInstance> eq =
                            EnergyQuantum.createAdd(fLink, sLink, addition,
                                    timeSlice, ec, "DaShmLinkStructure + "
                                            + linkType);
                    retval.add(eq);
                }
            }
