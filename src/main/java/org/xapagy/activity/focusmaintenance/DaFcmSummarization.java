...
public class DaFcmSummarization extends AbstractDaFocusIterator {
    ...
    protected void applyFocusVi(VerbInstance fvi, double time) {
        ...
        double summarizationEnergy =
                fc.getEnergy(fvi, EnergyColors.FOCUS_SUMMARIZATION_VI);
        ...
        if (SummarizationHelper.isClosedSummarization(agent, fvi)) {
            ...
            EnergyQuantum<VerbInstance> eq =
                    EnergyQuantum
                            .createAdd(fvi, additiveChange, time,
                                    EnergyColors.FOCUS_VI,
                                    "DaFcmSummarization + FocusSummarization to Focus increase");
            fc.applyViEnergyQuantum(eq);
            double decay = 0.5;
            EnergyQuantum<VerbInstance> eqDecay =
                    EnergyQuantum
                            .createMult(fvi, decay, time,
                                    EnergyColors.FOCUS_SUMMARIZATION_VI,
                                    "DaFcmSummarization + FocusSummarization to Focus decay");
        ...
        if (SummarizationHelper.isOpenSummarization(agent, fvi)) {
            ...
            if (!hasActiveLink) {
                ...
                EnergyQuantum<VerbInstance> eqDecay =
                        EnergyQuantum
                                .createMult(fvi, 0.0, time,
                                        EnergyColors.FOCUS_SUMMARIZATION_VI,
                                        "DaFcmSummarization + expiring");
                fc.applyViEnergyQuantum(eqDecay);
                for (VerbInstance vibegin : begins.getParticipants()) {
